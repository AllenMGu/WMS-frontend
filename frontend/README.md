...
当前 SPA 覆盖货物、仓库库位、合作方、药品批次、采购收货、养护、温湿度、盘点、销售发运、运输签收、退货、不合格品、召回、追溯、用户岗位、LDAP、电子签名、审计和运维合规；首营管理支持把供应商与其获准供货品种建立有效期受控的关联，提供 CSV 模板、最多 1000 行的原子批量导入以及待审批/按批准阈值临期/过期预警，经独立质量批准后，采购建单只显示该供应商的有效品种；“我的质量任务”提供本人 CAPA 与培训闭环，“老 GSP 历史归档”提供受控迁移、独立核对、只读查询、打印和导出。
...
- 受控附件上传助手（`uploadControlledFile`/`bindControlledFileInput`）已在资质文件、供应商-品种授权、
  药品注册文件、承运商文件四类表单接入：选文件→上传→自动填入 `gspf:` 引用；弹窗关闭走 guarded
  cleanup，未绑定对象自动停用（需要后端 WMS 受控附件接口与 `ATTACHMENT_POLICY`）。
- 报表中心页（`assets/js/pages/reports.js`，NAV：系统与合规 → 业务报表）：目录（正式/开发预览徽标）→
  分页查询 → 受控打印 / 生成开发预览件 → 打印预览与哈希校验 → 打印记录台账（服务端分页）。
  静态资源缓存版本号：`20260906-reports`。
