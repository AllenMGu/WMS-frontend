                ...
                <table class="data-table">
                    ...
                    <tbody>${records.map(r => `
                        <tr>
                            ...
                            <td class="actions">
                                ${r.status === 'PENDING_APPROVAL' && hasAnyGspRole('QUALITY_MANAGER', 'QUALITY_REVIEWER') ? `<button class="btn btn-link btn-sm" data-action="disposition.approveNC" data-arg1="${r.id}"><i class="fa fa-gavel"></i> 批准处置</button>` : ''}
                                ${r.status === 'PENDING_APPROVAL' && hasAnyGspRole('QUALITY_MANAGER', 'QUALITY_REVIEWER') ? `<button class="btn btn-link btn-sm" data-action="disposition.rejectNC" data-arg1="${r.id}"><i class="fa fa-times"></i> 驳回登记</button>` : ''}
                                ${r.status === 'APPROVED' && r.approved_disposition === 'DESTROY' && hasAnyGspRole('WAREHOUSE_CUSTODIAN') ? `<button class="btn btn-link btn-sm" data-action="disposition.destroyNC" data-arg1="${r.id}"><i class="fa fa-fire"></i> 监督销毁</button>` : ''}
                            </td>
                ...
                <table class="data-table">
                    ...
                    <tbody>${returns.map(r => `
                        <tr>
                            ...
                            <td class="actions">
                                ${r.status === 'DRAFT' && hasAnyGspRole('PROCUREMENT') ? `<button class="btn btn-link btn-sm" data-action="disposition.submitPR" data-arg1="${r.id}"><i class="fa fa-paper-plane"></i> 提交</button>` : ''}
                                ${r.status === 'DRAFT' && hasAnyGspRole('PROCUREMENT') ? `<button class="btn btn-link btn-sm" data-action="disposition.cancelPR" data-arg1="${r.id}"><i class="fa fa-ban"></i> 取消</button>` : ''}
                                ${r.status === 'SUBMITTED' && hasAnyGspRole('QUALITY_MANAGER', 'QUALITY_REVIEWER') ? `<button class="btn btn-link btn-sm" data-action="disposition.approvePR" data-arg1="${r.id}"><i class="fa fa-check"></i> 批准</button>` : ''}
                                ${r.status === 'SUBMITTED' && hasAnyGspRole('QUALITY_MANAGER', 'QUALITY_REVIEWER') ? `<button class="btn btn-link btn-sm" data-action="disposition.rejectPR" data-arg1="${r.id}"><i class="fa fa-times"></i> 驳回</button>` : ''}
                                ${r.status === 'APPROVED' && hasAnyGspRole('WAREHOUSE_CUSTODIAN') ? `<button class="btn btn-link btn-sm" data-action="disposition.dispatchPR" data-arg1="${r.id}"><i class="fa fa-truck"></i> 退供发运</button>` : ''}
                            </td>
