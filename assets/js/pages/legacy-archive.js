...
(function () {
    ...
    function render() {
        content().innerHTML = `
            ...
            <div class="card mb-3"><div class="card-header"><span class="card-title">迁移与核对批次</span>${canPrepare() ? '<button class="btn btn-primary" id="newBatch">新建迁移批次</button>' : ''}</div><div class="card-body p-0 table-wrap"><table class="data-table"><thead><tr><th>批次</th><th>来源</th><th>映射</th><th>预期/导入/重复</th><th>汇总摘要</th><th>保留至</th><th>状态</th><th class="actions">操作</th></tr></thead><tbody>${batches.map(b => `<tr><td>${esc(b.batch_no)}</td><td>${esc(b.source_system)}<br><small>${esc(b.source_instance)}</small></td><td>${esc(b.mapping_version)}</td><td>${b.expected_record_count}/${b.imported_record_count}/${b.duplicate_record_count}</td><td><code>${esc((b.aggregate_sha256 || '-').slice(0, 12))}</code></td><td>${fmtD(b.retention_until)}</td><td>${statusBadge(b.status)}</td><td class="actions">${b.status === 'DRAFT' && canValidate() ? `<button class="btn btn-link btn-sm" data-action="legacy-archive.validateBatch" data-arg1="${b.id}">独立验证</button>` : ''}${['VALIDATED','IMPORTED'].includes(b.status) && canPrepare() ? `<button class="btn btn-link btn-sm" data-action="legacy-archive.importFile" data-arg1="${b.id}">导入 JSON</button>` : ''}${b.status === 'IMPORTED' && canReconcile() ? `<button class="btn btn-link btn-sm" data-action="legacy-archive.reconcile" data-arg1="${b.id}">独立核对</button>` : ''}${b.status === 'RECONCILED' ? `<button class="btn btn-link btn-sm" data-batch-action="search" data-id="${b.id}">查询</button><button class="btn btn-link btn-sm" data-batch-action="export" data-id="${b.id}" data-batch-no="${esc(b.batch_no)}">导出</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="8"><div class="empty-state">暂无历史迁移批次</div></td></tr>'}</tbody></table></div></div>
            <div class="card"><div class="card-header"><span class="card-title">只读历史记录</span><div><input id="archiveSearch" class="input-field" style="display:inline-block;width:220px" placeholder="编号、标题或关键词"><button class="btn btn-secondary ml-2" id="searchAll">检索</button><button class="btn btn-secondary ml-2" data-action="window.print">打印</button></div></div><div id="archiveRecords" class="card-body"><div class="empty-state">请选择已核对批次，或输入关键词检索</div></div></div>`;
        ...
        content().querySelectorAll('[data-batch-action]').forEach(btn => {
            const id = Number(btn.getAttribute('data-id'));
            const action = btn.getAttribute('data-batch-action');
            if (action === 'export') {
                btn.addEventListener('click', () => exportBatch(id, btn.getAttribute('data-batch-no') || ''));
            } else if (action === 'search') {
                btn.addEventListener('click', () => searchBatch(id));
            }
        });
    }
