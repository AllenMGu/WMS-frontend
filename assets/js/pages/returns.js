...
function renderTable() {
    ...
    tbody.innerHTML = returns.map(r => `
        <tr>
            ...
            <td class="actions">
                ${(r.items || []).filter(i => i.inspection_status === 'PENDING').map(i => `<button class="btn btn-link btn-sm" onclick="inspectReturnItem(${r.id}, ${i.id})"><i class="fa fa-search"></i> 检验</button>`).join('')}
            </td>
