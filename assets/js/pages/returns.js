...
(function () {
    ...
    function renderTable() {
        ...
        <tr>
            ...
            <td class="actions">
                ${r.status === 'PENDING_INSPECTION' && hasAnyGspRole('RETURNS_RECEIVER', 'WAREHOUSE_CUSTODIAN', 'QUALITY_MANAGER', 'QUALITY_REVIEWER') ? `<button class="btn btn-link btn-sm" data-action="returns.cancelReturn" data-arg1="${r.id}"><i class="fa fa-ban"></i> 取消</button>` : ''}
                ${hasAnyGspRole('QUALITY_MANAGER', 'QUALITY_REVIEWER') ? (r.items || []).filter(i => i.inspection_status === 'PENDING').map(i => `<button class="btn btn-link btn-sm" data-action="returns.inspectReturnItem" data-arg1="${r.id}" data-arg2="${i.id}"><i class="fa fa-search"></i> 检验</button>`).join('') : ''}
            </td>
