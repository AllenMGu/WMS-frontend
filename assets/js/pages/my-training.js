        ...
        content().innerHTML = `
            ...
            <div class="card">
                ...
                <div class="card-body p-0 table-wrap">
                    ...
                    <tbody>${records.map(t => `<tr><td>${esc(t.training_no)}</td><td>${esc(t.subject)}</td><td>${esc(t.training_type)}</td><td>${esc(t.requirement_ref)}</td><td>${fmtD(t.planned_date)}</td><td>${t.score ?? '-'}</td><td>${esc(t.result || '-')}</td><td>${fmtD(t.valid_to)}</td><td>${statusBadge(t.status)}</td><td class="actions">${t.status === 'PLANNED' ? `<button class="btn btn-link btn-sm" data-action="my-training.completeTraining" data-arg1="${t.id}"><i class="fa fa-check"></i> 本人确认</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="10"><div class="empty-state">暂无本人培训记录</div></td></tr>'}</tbody></table>
                </div>
            </div>`;
    }
