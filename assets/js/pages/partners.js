...
(function () {
    ...
    function openSupplierProductModal(partnerId, goodsId = null) {
        ...
        modal.querySelector('#spaRef').closest('.form-row').insertAdjacentHTML('afterend',
            `<div class="form-group"><label class="form-label">上传受控授权文件（推荐）</label><input type="file" id="spaFile" class="input-field" accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.xls,.xlsx,.zip,.csv,.txt"><button type="button" id="spaCancel" class="btn btn-secondary btn-sm mt-2">取消上传并停用</button><div class="text-xs text-gray-500" id="spaFileInfo">选择文件自动填入 引用/SHA-256/大小；上传后可再选更换</div></div>`);
        const spaCtl = bindControlledFileInput(modal, { fileSel: '#spaFile', infoSel: '#spaFileInfo', refSel: '#spaRef', hashSel: '#spaHash', sizeSel: '#spaSize', purpose: 'SUPPLIER_PRODUCT_AUTHORIZATION', submitSel: '#spaSubmit' });
        registerControlledCloseGuard(modal, spaCtl);
        modal.querySelector('#spaCancel').addEventListener('click', () => { spaCtl.cancel(); });
    ...
    function openDocModal(partnerId, docs, onAdd) {
        ...
        modal.querySelector('#dRef').closest('.form-group').insertAdjacentHTML('beforebegin',
            `<div class="form-group"><label class="form-label">上传受控附件（推荐）</label><input type="file" id="dFile" class="input-field" accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.xls,.xlsx,.zip,.csv,.txt"><button type="button" id="dCancel" class="btn btn-secondary btn-sm mt-2">取消上传并停用</button><div class="text-xs text-gray-500" id="dFileInfo">选择文件自动上传并填入 file_ref；上传后可再选更换</div></div>`);
        const dCtl = bindControlledFileInput(modal, { fileSel: '#dFile', infoSel: '#dFileInfo', refSel: '#dRef', purpose: 'PARTNER_DOCUMENT', submitSel: '#dSubmitBtn' });
        registerControlledCloseGuard(modal, dCtl);
        modal.querySelector('#dCancel').addEventListener('click', () => { dCtl.cancel(); });
